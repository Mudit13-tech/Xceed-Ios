...
export default function AssignmentResults() {
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      setData(await lmApi.assignmentResults(classId, assignmentId));
    } catch (err) {
      setError(err);
    } finally {
      setLoading(false);
  ...
  const applyAdjustment = async (attempt) => {
    ...
    try {
      const result = await lmApi.adjustAssignmentAttempt(classId, attempt._id, value, entry.feedback || '');
  ...
  const { assignment, attempts, summary, perAnswer } = data;
  ...
  return (
    <Box>
      ...
      <Text fontSize="sm" color="lmFg.muted" mb={4}>
        {assignment.questions.length} questions · {assignment.totalMarks} marks · every student receives their
        own values
      </Text>
