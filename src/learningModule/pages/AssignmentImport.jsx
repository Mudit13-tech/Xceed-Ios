 ...
 * merge produces an unpublished assignment that still has to be reviewed and
...
function DraftQuestion({ classId, draftId, question, index, onChanged, selected, onSelect }) {
  ...
  const save = (changes) =>
    run(() => lmApi.updateAssignmentImportQuestion(classId, draftId, question._id, changes), 'Could not save');
...
function PreviewPanel({ classId, draftId, questionIds }) {
  ...
  const roll = async () => {
    ...
    setProblem(null);
    try {
      const result = await lmApi.previewAssignmentImport(classId, draftId, { count: 3, questionIds });
...
export default function AssignmentImport() {
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      const loaded = await lmApi.getAssignmentImport(classId, draftId);
      setDraft(loaded);
  ...
  const merge = async () => {
    ...
    try {
      ...
      const result = await lmApi.mergeAssignmentImport(classId, draftId, { questionIds });
      toast({ status: 'success', title: 'Merged into a new assignment', duration: 5000 });
      // Straight into the ordinary editor: from here it is a assignment like any
      // other, and publishing it is a separate, deliberate act.
      navigate(`/learning/class/${classId}/assignment/${result.assignment._id}/edit`);
    } catch (err) {
      ...
      <VStack align="stretch" spacing={3} mt={4}>
        ...
        <Flex gap={3} align="center" wrap="wrap">
          <Button
            ...
            onClick={merge}
            isLoading={merging}
            isDisabled={!chosen || Boolean(draft.assignmentId)}
          >
        ...
        {draft.assignmentId && (
          <Alert status="success" borderRadius="md" fontSize="sm">
            ...
            <Button
              ...
              variant="link"
              ml={1}
              onClick={() => navigate(`/learning/class/${classId}/assignment/${draft.assignmentId}/edit`)}
            >
              Open the assignment
            </Button>
          </Alert>
