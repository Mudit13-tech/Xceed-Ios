...
// The invigilation pieces the quiz card and this page share. The panel
// itself now hangs off the card in the quiz list — where staff are when an
// exam starts — and this page keeps the deeper monitor built from the same
// parts.
import {
  AttemptFlags,
  CardFact,
  ...
  nameOf,
  useSecondTick,
} from '../components/liveExam';
import { useTerminationAlerts } from '../../mobile/useTerminationAlerts';
...
export default function QuizResults() {
  ...
  const announceTerminations = useTerminationAlerts({ classId, onReload: () => load(true) });
  ...
  const load = useCallback(
    async (quiet = false) => {
      ...
      try {
        ...
        announceTerminations(fetched, quiet);
  ...
  return (
    <Box>
      <Flex justify="space-between" align="flex-start" mb={4} gap={3} wrap="wrap">
        ...
        <HStack>
          {/* No live-control button here any more. Mid-exam a teacher is on
              the quiz list watching the card, not on an analytics page, so the
              panel opens from there; what stays here is the monitor tab and the
              per-student fixes, which are what this page is for. */}
