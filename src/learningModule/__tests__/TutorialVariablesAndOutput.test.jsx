...
/**
 * Two authoring complaints about parameterised tutorials, and their fixes.
 *
...
// under test here, only what TutorialEditor does with the text it emits.
...
const previewTutorial = vi.fn();
const updateTutorial = vi.fn().mockResolvedValue({});
vi.mock('../api/lmApi', () => ({
  default: {
    getTutorial: (...a) => getTutorial(...a),
    previewTutorial: (...a) => previewTutorial(...a),
    formulaReference: vi.fn().mockRejectedValue(new Error('no reference')),
    validateFormula: vi.fn().mockResolvedValue({ ok: true, error: null }),
    updateTutorial: (...a) => updateTutorial(...a),
    publishTutorial: vi.fn(),
  },
}));

// A backend that remembers: updateTutorial stores the payload and getTutorial
// serves it back, which is what the editor's save-then-reload cycle needs.
const withPersistence = (base) => {
  let stored = base;
  getTutorial.mockImplementation(async () => JSON.parse(JSON.stringify(stored)));
  updateTutorial.mockImplementation(async (classId, tutorialId, body) => {
    stored = { ...stored, ...body };
    return stored;
  });
};

const tutorialWith = (question) => ({
  _id: 't1',
  title: 'Ohm practice',
  description: '',
  topicId: null,
  published: false,
  totalMarks: 1,
  settings: { dueDate: null, attemptsAllowed: 1, passPercent: 40 },
  questions: [
    {
      _id: 'q1',
      prompt: '',
...
describe('typing a placeholder declares the variable', () => {
  ...
  it('adds a variable row for a {{name}} typed into the prompt', async () => {
    getTutorial.mockResolvedValue(tutorialWith({}));
  ...
  it('does not edit a question just because it was opened', async () => {
    ...
    getTutorial.mockResolvedValue(tutorialWith({ prompt: 'A {{R}} ohm resistor', variables: [] }));
    const { default: TutorialEditor } = await import('../pages/TutorialEditor');
    renderWithProviders(<TutorialEditor />);
...
describe('the table builder', () => {
  it('drops a token at the cursor and keeps the table on the question', async () => {
    ...
    withPersistence(
      tutorialWith({ prompt: 'Given: ', variables: [{ name: 'R', type: 'integer', min: 1, max: 9, step: 1 }] }),
    );
    const { default: TutorialEditor } = await import('../pages/TutorialEditor');
    renderWithProviders(<TutorialEditor />);
    ...
    // Saving reloads the tutorial, so the fake backend has to hand back what it
    // was given — otherwise the reload would wipe the table and hide the very
    // bug this test is about.
    await waitFor(() => expect(updateTutorial).toHaveBeenCalled());
    ...
    await waitFor(() => expect(updateTutorial).toHaveBeenCalled());
    const saved = updateTutorial.mock.calls.at(-1)[2];
...
describe('the per-question preview', () => {
  it('shows this question only, with its sub-questions and their answers', async () => {
    ...
    previewTutorial.mockResolvedValue({
      warnings: [],
      samples: [
        {
          label: 'Student 1',
...
describe('adding a question', () => {
  it('opens the question that was just added', async () => {
    getTutorial.mockResolvedValue(tutorialWith({ prompt: 'first' }));
    const { default: TutorialEditor } = await import('../pages/TutorialEditor');
    renderWithProviders(<TutorialEditor />);
