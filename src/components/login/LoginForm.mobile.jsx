/**
 * The sign-in screen, on a device.
 *
 * A genuine fork, not a patch. The web form asks for an email and a password
 * once; this one is the front door to a saved-account list, a PIN, and a
 * fingerprint prompt, and it has to hand a restored session to a deep link
 * that arrived while the app was closed. That reorganises the component
 * around state the original does not have, which is why merging the two kept
 * failing: AMS was not editing lines we had changed, it was editing lines we
 * had moved.
 *
 * The AMS copy beside this one is left byte-identical to upstream so it always
 * merges cleanly. Nothing imports it — build/mobileOverrides.js redirects
 * every import of ./LoginForm here — so upstream changes now arrive silently
 * instead of as a conflict. Run npm run drift to see what has arrived and
 * decide what to port; the drift report is the only thing standing between
 * this file and the slow rot that turned getenvironment.js into a copy of a
 * version of AMS from months earlier.
 */
